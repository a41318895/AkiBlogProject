...
const Typeit = (isAimee,emId) =>{
    var theater = new TheaterJS();
        //使用TheaterJS，可以建立多個角色，每個角色都有自己的“exp”，使用“exp”可以互相“communicate”。
        //上面的code描述了一個新的角色，called “Vader”，它的“exp”為0.8（必是0-1之間），它的voice是 “#vader”。voice用於打印的文字，Vader是一個html Element。
        /***voice可為兩種type：
            一個HTML元素（或一個CSS selector），元素的innerHTML將被用於設置voice。
            用4個參數call的函數：
            newValue：新的speech值。
            newChar：新的打印string。
            prevChar：前一個string。
        ...
        // conosle.log()
        theater.on("*", function (eventName, originalEvent, sceneName, arg) { // do something

                })
                .on("say:start, erase:start", function (eventName) { // Adding flash insert symbol
                    var self    = this,current = self.current.voice; 
                    self.utils.addClass(current, "saying");
                })
                .on("say:end, erase:end", function (eventName) { // Deleting flash insert symbol
                    var self    = this,current = self.current.voice;
